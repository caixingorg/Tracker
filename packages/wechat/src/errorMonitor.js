...
export function initErrorMonitor(config, beaconSender) {
  ...
  function handleError(error, source) {
    ...
    // 格式化错误信息
    const formattedError = formatWechatError(error, source);
  ...
  if (config.debug) {
    console.log('[AutoTracker] WeChat Mini Program error monitor initialized');
  }
}
 ...
 */
function formatWechatError(error, source) {
  ...
  if (typeof error === 'string') {
    ...
    if (error.errMsg) {
      ...
      if (message.includes('request:fail')) {
        type = 'network_error';
        subType = 'request';
      } else if (message.includes('navigateTo:fail')) {
        type = 'navigation_error';
        subType = 'navigate';
      } else if (message.includes('setStorage:fail')) {
        type = 'storage_error';
        subType = 'set';
      } else if (message.includes('getStorage:fail')) {
        type = 'storage_error';
        subType = 'get';
      }
    }
